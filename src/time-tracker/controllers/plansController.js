import TimePlan from "../models/timePlan.js";
import TimeSession from "../models/timeSession.js";
import { asyncHandler } from "../utils/asyncHandler.js";
import { dayWindow, rangeWindow } from "../utils/timeRange.js";
import { classifyForUser } from "../utils/classifier.js";

export const listPlans = asyncHandler(async (req, res) => {
  const userId = req.auth0Id;
  const { date, tz, start, end, status } = req.query;

  let window;
  if (date) window = dayWindow(date, tz);
  else if (start && end) window = rangeWindow(start, end, tz);
  else return res.status(422).json({ success: false, message: "Provide date or start&end" });

  const q = {
    userId,
    $or: [
      { startTime: { $lt: window.end }, endTime: { $gt: window.start } },
      { startTime: { $gte: window.start, $lt: window.end } },
    ],
  };
  if (status) q.status = status;

  const plans = await TimePlan.find(q).sort({ startTime: 1 }).lean();
  
  // Ensure instanceDate is set for all plans (backward compatibility)
  const plansWithInstanceDate = plans.map(plan => {
    if (!plan.instanceDate && plan.startTime) {
      // Derive instanceDate from startTime
      const instanceDate = new Date(plan.startTime);
      instanceDate.setHours(0, 0, 0, 0);
      plan.instanceDate = instanceDate;
    }
    return plan;
  });
  
  res.json({ success: true, data: plansWithInstanceDate });
});

export const createPlan = asyncHandler(async (req, res) => {
  const userId = req.auth0Id;
  const body = req.body || {};
  const { taskId=null, taskTitle=null, groupTag="@personal", categoryId, startTime, endTime, notes="", isOverride=false } = body;

  if (!startTime || !endTime) return res.status(422).json({ success: false, message: "startTime and endTime are required" });
  if (new Date(endTime) <= new Date(startTime)) return res.status(422).json({ success: false, message: "endTime must be after startTime" });

  let cat = categoryId;
  if (!cat) {
    const guess = await classifyForUser(userId, taskTitle);
    cat = guess.categoryId;
  }

  // Extract instanceDate from startTime (normalize to start of day)
  const startDate = new Date(startTime);
  const instanceDate = new Date(startDate);
  instanceDate.setHours(0, 0, 0, 0);

  // If this is an override and taskId is provided, check if override already exists
  if (isOverride && taskId) {
    const existingOverride = await TimePlan.findOne({
      userId,
      taskId,
      instanceDate: instanceDate
    });

    if (existingOverride) {
      // Update existing override instead of creating duplicate
      const updated = await TimePlan.findOneAndUpdate(
        { _id: existingOverride._id, userId },
        {
          taskTitle,
          categoryId: cat,
          startTime,
          endTime,
          notes,
          isOverridden: true, // Mark as override
        },
        { new: true }
      );
      return res.status(200).json({ success: true, data: updated });
    }
  }

  const plan = await TimePlan.create({ 
    userId, 
    taskId, 
    taskTitle, 
    groupTag, 
    categoryId: cat, 
    startTime, 
    endTime, 
    notes, 
    status: "scheduled",
    instanceDate,
    isAutoGenerated: false,
    isOverridden: isOverride || false // Set isOverridden if this is an override
  });
  res.status(201).json({ success: true, data: plan });
});

export const patchPlan = asyncHandler(async (req, res) => {
  const userId = req.auth0Id;
  const { id } = req.params;
  const u = {};
  ["taskId","taskTitle","groupTag","categoryId","notes","status"].forEach(k => { if (k in req.body) u[k] = req.body[k]; });
  if ("startTime" in req.body) {
    u.startTime = new Date(req.body.startTime);
    // Update instanceDate if startTime changes
    const instanceDate = new Date(u.startTime);
    instanceDate.setHours(0, 0, 0, 0);
    u.instanceDate = instanceDate;
  }
  if ("endTime" in req.body) u.endTime = new Date(req.body.endTime);

  // Check if this is an auto-generated plan being edited
  const existingPlan = await TimePlan.findOne({ _id: id, userId });
  if (!existingPlan) return res.status(404).json({ success: false, message: "Plan not found" });

  // If plan is auto-generated and user is editing it, mark as overridden
  if (existingPlan.isAutoGenerated && !existingPlan.isOverridden) {
    u.isOverridden = true;
    // Preserve template values if not already set
    if (!u.templateStartTime && existingPlan.templateStartTime) {
      u.templateStartTime = existingPlan.templateStartTime;
    }
    if (!u.templateEndTime && existingPlan.templateEndTime) {
      u.templateEndTime = existingPlan.templateEndTime;
    }
  }

  const doc = await TimePlan.findOneAndUpdate({ _id: id, userId }, { $set: u }, { new: true });
  if (!doc) return res.status(404).json({ success: false, message: "Plan not found" });
  if (doc.endTime <= doc.startTime) return res.status(422).json({ success: false, message: "endTime must be after startTime" });
  res.json({ success: true, data: doc });
});

export const deletePlan = asyncHandler(async (req, res) => {
  const userId = req.auth0Id;
  const { id } = req.params;
  const doc = await TimePlan.findOneAndDelete({ _id: id, userId });
  if (!doc) return res.status(404).json({ success: false, message: "Plan not found" });
  res.json({ success: true, message: "Deleted" });
});

// Convert a plan into a running session (start now)
export const startFromPlan = asyncHandler(async (req, res) => {
  const userId = req.auth0Id;
  const { id } = req.params;
  const plan = await TimePlan.findOne({ _id: id, userId });
  if (!plan) return res.status(404).json({ success: false, message: "Plan not found" });

  // Stop any running session
  await TimeSession.updateMany({ userId, endTime: null }, { $set: { endTime: new Date() } });

  const session = await TimeSession.create({
    userId,
    taskId: plan.taskId,
    taskTitle: plan.taskTitle,
    groupTag: plan.groupTag,
    categoryId: plan.categoryId,
    startTime: new Date(),
    endTime: null,
    source: "timer",
    notes: plan.notes || "",
  });

  plan.status = "in_progress";
  plan.sessionId = session._id;
  await plan.save();

  res.status(201).json({ success: true, data: { plan, session } });
});

// Mark plan done by creating a completed manual session that matches planned window (if no session yet)
export const completePlan = asyncHandler(async (req, res) => {
  const userId = req.auth0Id;
  const { id } = req.params;
  const plan = await TimePlan.findOne({ _id: id, userId });
  if (!plan) return res.status(404).json({ success: false, message: "Plan not found" });

  let session = null;
  if (!plan.sessionId) {
    session = await TimeSession.create({
      userId,
      taskId: plan.taskId,
      taskTitle: plan.taskTitle,
      groupTag: plan.groupTag,
      categoryId: plan.categoryId,
      startTime: plan.startTime,
      endTime: plan.endTime,
      source: "manual",
      notes: plan.notes || "",
    });
    plan.sessionId = session._id;
  }
  plan.status = "done";
  await plan.save();

  res.json({ success: true, data: { plan, session } });
});